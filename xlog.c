...
char *hp_get_base_filename(char *filename) {
	char *ptr;
	int   found = 0;
		...
		if (func) {
			...
			*/
			if (add_filename){
				char *filename;
				int   len;
				filename = hp_get_base_filename((char *)(curr_func->op_array).filename);
...
void(*zend_execute_ex_old)(zend_execute_data *execute_data TSRMLS_DC);
void(*zend_execute_internal_old)(zend_execute_data *execute_data_ptr, struct _zend_fcall_info *fci, int return_value_used TSRMLS_DC);
...
{
	struct my_trace *mytrace;
	struct my_trace **mytrace_find;
	HashTable *table = XLOG_G(func_traces);
	if (table == NULL){
		ALLOC_HASHTABLE(XLOG_G(func_traces));
		zend_hash_init(XLOG_G(func_traces), 8, NULL, NULL, 0);
	}
	table = XLOG_G(func_traces);
	char *fun = hp_get_function_name(execute_data->op_array);
	if (fun){
		if (zend_hash_find(XLOG_G(func_traces), fun, strlen(fun) + 1, (void **)&mytrace_find) == SUCCESS){
			(*mytrace_find)->recr++;
			zend_hash_update(XLOG_G(func_traces), fun, strlen(fun) + 1, mytrace_find, sizeof(struct my_trace **), NULL);
		}
		else{
			mytrace = (struct my_trace *) emalloc(sizeof(struct my_trace));
			mytrace->recr = 1;
			mytrace->start = time(NULL);
			zend_hash_add(XLOG_G(func_traces), fun, strlen(fun) + 1, &mytrace, sizeof(struct my_trace **), NULL);
		}
	}
	zend_execute_ex_old(execute_data TSRMLS_CC);
	if (fun){
		if (zend_hash_find(XLOG_G(func_traces), fun, strlen(fun) + 1, (void **)&mytrace_find) == SUCCESS){
			(*mytrace_find)->recr--;
			if ((*mytrace_find)->recr == 0){
				if (time(NULL) - ((*mytrace_find)->start) > 1){
					php_printf("func:%s(%d)\n", fun, time(NULL) - ((*mytrace_find)->start));
				}
				zend_hash_del(XLOG_G(func_traces), fun, strlen(fun) + 1);
				efree(*mytrace_find);
			}
			else{
				zend_hash_update(XLOG_G(func_traces), fun, strlen(fun) + 1, mytrace_find, sizeof(struct my_trace **), NULL);
...
void _zend_execute_internal(zend_execute_data *execute_data_ptr, struct _zend_fcall_info *fci, int return_value_used TSRMLS_DC)
{
	php_printf("zend_execute_internal\n");
	zend_execute_internal_old(execute_data_ptr, fci, return_value_used TSRMLS_CC);
}
...
PHP_MINIT_FUNCTION(xlog)
{
	...
	zend_execute_ex_old = zend_execute_ex;
...
PHP_MSHUTDOWN_FUNCTION(xlog)
{
	...
	*/
	...
	zend_execute_ex = zend_execute_ex_old  ;
	if (zend_execute_internal_old){
